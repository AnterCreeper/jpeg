...
void idct(int16_t* block, unsigned char* output, int id){
	...
	for(int i = 0; i < 8; i++)
		for(int j = 0; j < 8; j++){
			buf[i][j]  = block[j]    * (dct_mat[0][i] >> 8);
			buf[i][j] += block[8+j]  * (dct_mat[1][i] >> 8);
			buf[i][j] += block[16+j] * (dct_mat[2][i] >> 8);
			buf[i][j] += block[24+j] * (dct_mat[3][i] >> 8);
			buf[i][j] += block[32+j] * (dct_mat[4][i] >> 8);
			buf[i][j] += block[40+j] * (dct_mat[5][i] >> 8);
			buf[i][j] += block[48+j] * (dct_mat[6][i] >> 8);
			buf[i][j] += block[56+j] * (dct_mat[7][i] >> 8);
		}
	int16_t buf_1[8][8];
	for(int i = 0; i < 8; i++)
		for(int j = 0; j < 8; j++) buf_1[i][j] = buf[i][j] >> 7;
