...
//void yuv2rgb(unsigned char* rgb, float y, float u, float v){
...
int main(){
	unsigned char mem[68][120][4][64*3];
        FILE *fsrc = fopen("tb_output_mat.bin","r");
		...
		for (int i = 0; i < 6; i++) {
			int16_t block[64];
			for (int j = 0; j < 8; j++)
			fscanf(fsrc, "%x %x %x %x %x %x %x %x\n",
				&block[8*j], &block[8*j+1], &block[8*j+2], &block[8*j+3],
				&block[8*j+4], &block[8*j+5], &block[8*j+6], &block[8*j+7]);
